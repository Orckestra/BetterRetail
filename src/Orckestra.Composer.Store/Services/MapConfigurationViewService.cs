...
namespace Orckestra.Composer.Store.Services
{
    public class MapConfigurationViewService : IMapConfigurationViewService
    {
            ...
            GetMapConfigurationViewModelParam param)
        {
            ...
            {
                var stores = await StoreRepository.GetStoresAsync(new GetStoresParam { Scope = param.Scope, IncludeExtraInfo = false, CultureInfo = param.CultureInfo }).ConfigureAwait(false);

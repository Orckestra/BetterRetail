...
if (Test-Path $depoymentFolder) {
    ...
    Copy-Item -Path (Get-Item -Path "$srcFolder\Orckestra.Composer.Website\UI.Package\*" -Exclude ('Sass')).FullName -Destination "$depoymentFolder\UI.Package" -Recurse -Force
	...
	Copy-Item -Path "$srcFolder\Orckestra.Composer.Website\App_Data" -Destination "$depoymentFolder" -Recurse -Force

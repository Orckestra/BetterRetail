...
<xsl:stylesheet version="1.0"
    ...
    <xsl:output method="xml"
                indent="yes" />
    <xsl:variable name="ComposerSectionGroup"
                  xml:space="preserve">
        <sectionGroup name="composer"
                      type="System.Configuration.ConfigurationSectionGroup, System.Configuration">
            <section name="caching"
                     type="Orckestra.Composer.Caching.CacheConfiguration, Orckestra.Composer" />
        </sectionGroup>
    </xsl:variable>
    <xsl:variable name="ConfigBuildersSection"
                  xml:space="preserve">
    ...
    <xsl:variable name="ComposerSection"
                  xml:space="preserve">
        <composer>
            <caching configSource="App_Config\Caching.config" />
        </composer>
    </xsl:variable>
    <xsl:variable name="ConfigBuilders"
                  xml:space="preserve">
        <configBuilders>
            <builders>
                <add name="EnvAppSettings"
                     mode="Greedy"
                     prefix="AppSettings_"
                     stripPrefix="true"
                     type="Orckestra.Composer.Configuration.EnvironmentVariablesKeyValueConfigurationBuilder,Orckestra.Composer" />
                <add name="EnvExpManSettings"
                     mode="Greedy"
                     prefix="ExperienceManagement_"
                     stripPrefix="true"
                     type="Orckestra.Composer.Configuration.EnvironmentVariablesKeyValueConfigurationBuilder,Orckestra.Composer" />
            </builders>
        </configBuilders>
    </xsl:variable>
    <xsl:variable name="AppSettings"
                  xml:space="preserve">
        <appSettings configBuilders="EnvAppSettings">
            ...
            <add key="Environment"
                 value="dev" />
            <add key="Hostname"
                 value="composer-c1-cm-dev.orckestra.local" />
            <add key="Variation"
                 value="" />
            <add key="APPINSIGHTS_INSTRUMENTATIONKEY"
                 value="" />
            <add key="HandelbarsCompiler"
                 value="true" />
            <add key="Composer.DefaultScope"
                 value="Canada" />
            <add key="CC1.DeploymentToken"
                 value="***REMOVED***" />
            <add key="UrlAlias::UseCountEnabled"
                 value="false" />
            <add key="AutoImageResizing.ImageWidthBreakpoints"
                 value="280, 680, 1130" />
            <add key="AutoImageResizing.MaxWidth"
                 value="2200" />
            <add key="AutoImageResizing.ImageFormats"
                 value="image/webp, image/jpeg" />
            <xsl:comment> OWIN </xsl:comment>
            <add key="owin:AutomaticAppStartup"
                 value="false" />
            <xsl:comment> Application Names for edit order Ownership</xsl:comment>
            <add key="ApplicationNames"
                 value="oco,orchestrator,website,wfecm,wfecd" />
        </appSettings>
    </xsl:variable>
        ...
        <xsl:copy xml:space="preserve">
            ...
            <xsl:if test="not(configSections)">
                <configSections>
                    ...
                    <sectionGroup name="experienceManagement"
                                  type="System.Configuration.ConfigurationSectionGroup, System.Configuration">
            ...
            <xsl:if test="not(experienceManagement)"
                    xml:space="preserve">
    ...
    <xsl:template match="configuration/appSettings"
                  xml:space="preserve">
        <xsl:copy>
            <xsl:attribute name="configBuilders">EnvAppSettings</xsl:attribute>
            <xsl:apply-templates select="@*[name(.)!='configBuilders']" />
            <xsl:apply-templates select="node()" />
    ...
    <xsl:template match="configuration/system.web"
        ...
        <xsl:copy>
            ...
            <membership defaultProvider="composer">
                <providers>
                    <add name="composer"
                         type="Orckestra.Composer.Providers.Membership.OvertureMembershipProvider, Orckestra.Composer" />
                </providers>
            </membership>
            <httpModules>
    ...
    <xsl:template match="configuration/system.web/caching/outputCacheSettings/outputCacheProfiles/add[@name='C1Page']"
                  xml:space="preserve">
    ...
    <xsl:template match="configuration/system.webServer/modules/add[@name='UrlRoutingModule']"
                  xml:space="preserve">
        <add name="UrlRewriteModule"
             type="Orckestra.Composer.CompositeC1.UrlRewriteModule, Orckestra.Composer.CompositeC1" />
        <add name="AntiCookieTamperingModule"
             type="Orckestra.Composer.HttpModules.AntiCookieTamperingModule, Orckestra.Composer" />
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" />
        </xsl:copy>
    ...
    <xsl:template match="/configuration/system.webServer/modules">
        <xsl:copy>
            ...
            <xsl:if test="not(add[@name='ContextPreservationHttpModule'])">
                <add name="ContextPreservationHttpModule"
                     type="Orckestra.Composer.CompositeC1.ContextPreservationHttpModule, Orckestra.Composer.CompositeC1" />
            </xsl:if>
            <xsl:apply-templates select="node()" />
            <xsl:if test="not(add[@name='ComposerRequestInterceptor'])">
                <add name="ComposerRequestInterceptor"
                     type="Orckestra.Composer.CompositeC1.RequestInterceptorHttpModule, Orckestra.Composer.CompositeC1" />
            </xsl:if>

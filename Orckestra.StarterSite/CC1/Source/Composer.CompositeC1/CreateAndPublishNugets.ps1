...
function Get-Version() {
    ...
    elseif($Mode -ilike "PreRelease") {
        ...
        $nugetVersion = "$packageVersion-Stable"
    }
    else {
...
function Package-Nuget($NuVersion) {
    Write-Host "`r`n============================================================="
    ...
    foreach($nuspec in $nuspecs) {
        "Packing  [$nuspec]..."
        Invoke-NugetPackage -NugetExeLocation ".\.nuget" -NuspecFile .\NugetFiles\$nuspec -NuVersion $NuVersion 2>&1 >> "..\nugetpackage.log"
    }
    Get-Content "..\nugetpackage.log"
}

function Publish-Nuget($NuVersion) {
    Write-Host "`r`n============================================================="
    Write-Host "Publishing NuGets to '$NugetFeed'" -ForegroundColor Yellow
    Write-Host "=============================================================`r`n"
    
    "" > "..\nugetpush.log"
    $packages = [string[]](get-childitem .\*.$NuVersion.nupkg | % {$_.name})
...
function Update-CMPManifestVersion{
    param(
        [Parameter(Mandatory)]
        [String]$Version
    )
    Get-ChildItem -Filter 'Manifest.json' -Recurse | % {
        Write-Host "Updating manifest version $($_.FullName)"
        $Manifest = Get-Content -Path $_.FullName | Out-String | ConvertFrom-Json
        $Manifest.Version = $Version
        $Manifest | ConvertTo-Json | Out-File -Force -FilePath $_.FullName
    }
}
...
Update-CMPManifestVersion -Version $nugetVersion

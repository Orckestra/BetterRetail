...
namespace Orckestra.Composer.CompositeC1.Services.Cache
{
    internal class CacheService : ICacheService, IDisposable
    {
        ...
        public ICacheStore<K, V> GetStore<K, V>(string name, int maximumSize)
        {
            return GetOrCreateStore<K, V>(name, maximumSize);
        }

        public ICacheStore<K, V> GetStore<K, V>(string name)
        {
            return GetOrCreateStore<K, V>(name);
        }

        public ICacheStore<K, V> GetStoreWithDependencies<K, V>(string name, int maximumSize, params CacheDependentEntry[] dependentEntities)
        {
            return GetOrCreateStore<K, V>(name, dependentEntities, maximumSize);
        }

        public ICacheStore<K, V> GetStoreWithDependencies<K, V>(string name, params CacheDependentEntry[] dependentEntities)
        {
            return GetOrCreateStore<K, V>(name, dependentEntities);
        }

        private ICacheStore<K, V> GetOrCreateStore<K, V>(string name, int? maximumSize = null)
        {
            return (ICacheStore<K, V>)_cacheStore.GetOrAdd((name, typeof(K), typeof(V)), _ => CreateStore<K, V>(name, maximumSize));
        }

        private CacheStore<K, V> CreateStore<K, V>(string name, int? maximumSize = null)
        {
        ...
        private ICacheStore<K, V> GetOrCreateStore<K, V>(string name, CacheDependentEntry[] dependentEntities, int? maximumSize = null)
        {
            return (ICacheStore<K, V>)_cacheStore.GetOrAdd((name, typeof(K), typeof(V)), _ => CreateStore<K, V>(name, dependentEntities, maximumSize));
        }

        private DependentCacheStore<K, V> CreateStore<K, V>(string name, CacheDependentEntry[] dependentEntities, int? maximumSize = null)
        {
            if (maximumSize.HasValue)
            {

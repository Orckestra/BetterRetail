...
namespace Orckestra.Composer.Cart.Tests.Factory
{
    ...
    public class LineItemViewModelFactory_CreateViewModel
    {
        ...
        public void SetUp()
        {
            ...
            mapper.Setup(m => m.MapTo<LineItemDetailViewModel>(It.IsAny<LineItem>(), It.IsAny<CultureInfo>(), null)).Returns<LineItem, CultureInfo, string>(
                (li, ci, currencyIso) => new LineItemDetailViewModel { CurrentPrice = li.CurrentPrice.GetValueOrDefault().ToString(CultureInfo.InvariantCulture), DefaultPrice = li.DefaultPrice.GetValueOrDefault().ToString(CultureInfo.InvariantCulture) });

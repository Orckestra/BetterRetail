...
namespace Orckestra.Composer.Product.Tests.Services
{
    ...
    public class CategoryViewServiceGetCategoriesPathAsync
    {
        ...
        private Mock<IViewModelMapper> CreateViewModelMapperMock()
        {
            ...
            mapperMock.Setup(repo => repo.MapTo<CategoryViewModel>(It.Is<Category>(c => c.Id == "A"), It.IsAny<CultureInfo>(), null))
                .Returns(new CategoryViewModel { Id = "A" })
                .Verifiable();

            mapperMock.Setup(repo => repo.MapTo<CategoryViewModel>(It.Is<Category>(c => c.Id == "B"), It.IsAny<CultureInfo>(), null))
